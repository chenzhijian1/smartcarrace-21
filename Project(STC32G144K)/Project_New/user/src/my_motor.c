    ...
    motor_struct *sptr, int16 pwm_feedforward)
{
    ...
    /* Stop integrating into saturation while allowing recovery from it. */
...
{
    return motor_closed_loop_control_with_feedforward(sptr, 0);
}
...
void motor_set_feedforward_pwm(int16 pwm)
{
    ...
    pwm = (int16)motor_pwm_limit((int)pwm);
    next_index = (uint8)(motor_feedforward_active_index ^ 1U);
    motor_feedforward_pwm[next_index] = pwm;
    motor_feedforward_active_index = next_index;
}
...
int16 motor_get_feedforward_pwm(void)
{
    ...
    index = motor_feedforward_active_index;
    return motor_feedforward_pwm[index];
}

void motor_control(int16 speed_l, int16 speed_r)
{
    int16 pwm_feedforward;
    ...
    pwm_feedforward = motor_get_feedforward_pwm();
    motor_closed_loop_control_with_feedforward(
        &motor_left, pwm_feedforward);
    motor_closed_loop_control_with_feedforward(
        &motor_right, pwm_feedforward);
    ...
    motor_left.duty1 = (int16)motor_pwm_limit(
        (int)(motor_left.out_motor_pid + pwm_feedforward));
    motor_right.duty1 = (int16)motor_pwm_limit(
        (int)(motor_right.out_motor_pid + pwm_feedforward));
